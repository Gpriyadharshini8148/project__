...
test.describe('03 - Zip Code Verification', () => {
  ...
  async function completePrerequisites(context: any, explicitMobileNumber?: string) {
    ...
    if (page) {
      const isZipCode = await page.locator('text=/Zip Code Verification|Zipcode|Customer ZipCode|Pin Code Verification/i').first().isVisible({ timeout: 5000 }).catch(() => false);
      if (!isZipCode) {
...
test('Negative: Valid zip code 411001 with mismatched BFL branch 411014 — expects validation error.', async ({
  dealerSearchPage,
  appStatusPage,
  zipCodePage,
  page,
  ...
  await test.step('Enter zip 411001 with branch that belongs to 411014 (mismatch)', async () => {
    ...
    await expect(async () => {
      ...
      /BFL Branch option '411014' is not available|kindly select bfl branch|invalid branch|does not match/i
    );
    console.log('✓ Branch 411014 is unavailable for zip code 411001 — PASS');
  });
});

test('Negative: Proceed without entering DOB — expects mandatory field error.', async ({
  dealerSearchPage,
  appStatusPage,
  zipCodePage,
  page,
  ...
  await test.step('Fill all fields except DOB and click Proceed', async () => {
    ...
    await expect(async () => {
      ...
      /complete this field|date of birth|dob is required|please enter.*date|enter valid date|customer age cannot be lessthan 21 or greater than 68/i
    );
    console.log('✓ Validation error shown when DOB is missing — PASS');
  });
});

test('Existing customer: Gender is retained and does not block Proceed.', async ({
  dealerSearchPage,
  appStatusPage,
  zipCodePage,
  page,
  ...
  await test.step('Leave Gender unchanged and verify the existing-customer flow proceeds', async () => {
    ...
    await zipCodePage.proceed(testData['proceedbuttonvalue'] || 'Proceed');
    await expect(page.getByText(/MITC|Terms and Conditions/i).first()).toBeVisible({
      timeout: config.timeouts.element,
    });
    console.log('✓ Existing customer proceeded with retained Gender — PASS');
  });
});

test('Existing customer: POA Address Type defaults to Self.', async ({
  dealerSearchPage,
  appStatusPage,
  zipCodePage,
  page,
  ...
  await test.step('Omit POA input and verify the default value', async () => {
    ...
    const poaAddressType = page.getByRole('combobox', { name: /POA Address Type/i });
    if (await poaAddressType.isVisible({ timeout: 3000 }).catch(() => false)) {
      await expect(poaAddressType).toContainText(/Self/i);
    }
    console.log('✓ POA Address Type defaulted to Self — PASS');
  });
});

test('Positive: Preferred Language dropdown lists all expected language options.', async ({
  dealerSearchPage,
  appStatusPage,
  zipCodePage,
  page,
